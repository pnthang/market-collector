...
from ..data_scraper import vn_scraper
from ..data_scraper import yahoo_scraper
...
import tempfile
import time
from ..data_scraper.playwright_manager import BrowserManager
from ..db import SessionLocal
from ..db.models import IndexTracking, IndexPrediction, ModelMetadata
from .. import ml
...
def run(host: str = "0.0.0.0", port: int = 8080):
    ...
    uvicorn.run("app.system.health:app", host=host, port=port, log_level="info")
...
def api_ml_predictions(symbol: str, limit: int = 50, offset: int = 0, horizon: int = None, model_version: str = None, since: str = None, until: str = None):
    if not symbol:
        return JSONResponse(status_code=400, content={"ok": False, "reason": "missing symbol"})
    try:
        session = SessionLocal()
        index_code = f"US:{symbol}"
        q = session.query(IndexPrediction).filter_by(index_code=index_code)
        if horizon is not None:
            q = q.filter(IndexPrediction.horizon_days == int(horizon))
        if model_version:
            q = q.filter(IndexPrediction.model_version == model_version)
        from datetime import datetime
        if since:
            try:
                q = q.filter(IndexPrediction.generated_at >= datetime.fromisoformat(since))
            except Exception:
                pass
        if until:
            try:
                q = q.filter(IndexPrediction.generated_at <= datetime.fromisoformat(until))
            except Exception:
                pass
        total = q.count()
        rows = q.order_by(IndexPrediction.generated_at.desc()).offset(int(offset)).limit(int(limit)).all()
        out = []
        for r in rows:
            out.append({
        ...
        return JSONResponse(content={"ok": True, "symbol": symbol, "predictions": out, "total": total, "limit": int(limit), "offset": int(offset)})
    except Exception:
...
@app.post('/control/ml/train_async')
def control_ml_train_async(symbol: str):
    """Trigger async model training via Celery. Returns task id."""
    if not symbol:
        return JSONResponse(status_code=400, content={"ok": False, "reason": "missing symbol"})
    try:
        # import lazily to avoid circulars
        from ..ml import train_model_task
        task = train_model_task.delay(symbol)
        return JSONResponse(content={"ok": True, "task_id": task.id})
    except Exception:
        LOG.exception("Failed to enqueue training for %s", symbol)
        return JSONResponse(status_code=500, content={"ok": False})


@app.get('/api/ml/models')
def api_ml_models(symbol: str = None, limit: int = 50, offset: int = 0):
    try:
        session = SessionLocal()
        q = session.query(ModelMetadata)
        if symbol:
            q = q.filter(ModelMetadata.symbol == symbol)
        total = q.count()
        rows = q.order_by(ModelMetadata.trained_at.desc()).offset(int(offset)).limit(int(limit)).all()
        out = []
        for r in rows:
            out.append({
                'id': r.id,
        ...
        return JSONResponse(content={"ok": True, "models": out, "total": total, "limit": int(limit), "offset": int(offset)})
    except Exception:
...
from ..data_scraper.playwright_manager import BrowserManager
from ..db import SessionLocal
from ..db.models import IndexTracking
from .. import ml
